...
import { X, Lock, Mail, User, ShieldCheck, Sparkles, ArrowRight, CheckCircle, Crown, KeyRound, Building2, AlertTriangle } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose }) {
  ...
  const handleGoogleSignIn = (targetEmail) => {
    setErrorMessage('');
    const emailToAuth = targetEmail || googleEmailInput;

    const res = loginWithGoogle(emailToAuth);
    if (res.success) {
      onClose();
    } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-lg rounded-3xl bg-brand-surface dark:bg-brand-surface light:bg-white border border-brand-border dark:border-white/10 light:border-slate-200 shadow-2xl overflow-hidden p-6 sm:p-8 space-y-4">
        ...
        <div className="space-y-2">
          <button
            ...
            className="w-full py-3.5 px-4 rounded-2xl bg-white text-slate-900 hover:bg-slate-100 font-extrabold text-xs shadow-lg transition flex items-center justify-center space-x-2.5 border border-slate-300"
          >
            {/* Google SVG Icon */}
            ...
            <span>Continue with Google (Admin Authorized)</span>
          </button>

          {/* Google Account Whitelist Select Prompt */}
          {showGooglePrompt && (
            <div className="p-4 rounded-2xl bg-slate-900/90 light:bg-slate-100 border border-brand-violet/40 space-y-3 animate-fadeIn">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-white flex items-center space-x-1">
                  <ShieldCheck size={14} className="text-emerald-400" />
                  <span>Select Admin-Whitelisted Google Email</span>
                </span>
                <span className="text-[10px] text-amber-400 font-bold uppercase">Strict Whitelist</span>
              </div>

              {/* Pre-whitelisted Google Emails */}
              <div className="space-y-1.5">
                {authorizedEmails.slice(0, 3).map((a) => (
                  <button
                    key={a.email}
                    type="button"
                    onClick={() => handleGoogleSignIn(a.email)}
                    className="w-full p-2.5 rounded-xl bg-slate-800/80 hover:bg-brand-violet/20 border border-white/10 text-left transition flex items-center justify-between"
                  >
                    <div>
              ...
              <div className="pt-2 border-t border-white/10 space-y-2">
                <label className="text-[10px] font-bold uppercase text-slate-400">Or enter your Google Email address:</label>
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-brand-violet text-white text-xs font-bold hover:bg-brand-violet-hover transition"
                  >
                    Sign In
                  </button>
                </div>
        ...
        <div className="relative flex items-center my-2">
          ...
          <span className="flex-shrink mx-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Or Email Credentials</span>
          <div className="flex-grow border-t border-white/10" />
        </div>
        ...
        {/* ERROR MESSAGE DISPLAY (Bright Red for Unauthorized Emails) */}
        {errorMessage && (
          <div className="p-3.5 rounded-2xl bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-bold flex items-start space-x-2.5 animate-fadeIn">
